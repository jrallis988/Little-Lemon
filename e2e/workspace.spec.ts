...
test('landing page opens the workspace', async ({ page }) => {
  ...
  await expect(page).toHaveURL(/\/app(\/nate)?\/?$/);
  await expect(page.getByRole('heading', { name: 'Nate' })).toBeVisible();
});

test('employee switching, tabs, chat, and theme work', async ({ page }) => {
  await page.goto('/app/mia');
  ...
  await expect(
    page.getByText(/Payroll Manager|Recommended approach|Immediate next steps/i).first(),
