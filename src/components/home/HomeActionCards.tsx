...
const cards = [
  {
    ...
    bg: "bg-ocean",
    icon: (
      <svg viewBox="0 0 64 64" className="h-16 w-16 shrink-0" fill="none" stroke="white" strokeWidth="2" aria-hidden="true">
...
export function HomeActionCards() {
  return (
    <section className="bg-white py-s7" aria-label="Quick actions">
      <div className="wrap grid grid-cols-1 items-stretch gap-s4 md:grid-cols-3">
        {cards.map((card) => (
          <Link
            key={card.title}
            href={card.href}
            className={`flex h-[240px] flex-col items-center justify-center gap-s4 rounded-lg p-s6 text-center no-underline transition-transform hover:-translate-y-0.5 md:h-[260px] ${card.bg}`}
          >
            {card.icon}
            <span className="inline-flex min-h-11 items-center justify-center rounded-md border-2 border-white px-s5 py-2 text-base font-bold leading-snug text-white">
              {card.title}
            </span>
          </Link>
        ))}
